  ...
  {#if !selectedCalculator}
    ...
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {#each filteredCalculators as calc}
        <button class="card bg-base-100 shadow-md hover:shadow-lg hover:-translate-y-1 transition-all text-left" on:click={() => selectCalculator(calc)}>
          <div class="card-body p-4">
            ...
            <p class="text-xs text-base-content/60 line-clamp-2">{calc.description}</p>
          </div>
        </button>
      {/each}
